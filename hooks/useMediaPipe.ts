...
export const useMediaPipe = (videoRef: React.RefObject<HTMLVideoElement | null>, enabled: boolean = true) => {
  ...
  useEffect(() => {
    // Skip initialization if not enabled (e.g., using Touch Control mode)
    if (!enabled) {
    ...
    const setupMediaPipe = async () => {
      try {
        ...
        const landmarker = await HandLandmarker.createFromOptions(vision, {
          ...
          runningMode: "VIDEO",
          numHands: 2,
          minHandDetectionConfidence: 0.4,  // Lowered from 0.5 for iOS
          minHandPresenceConfidence: 0.4,   // Lowered from 0.5 for iOS
          minTrackingConfidence: 0.4        // Lowered from 0.5 for iOS
        });
    ...
    const startCamera = async () => {
      try {
        const stream = await navigator.mediaDevices.getUserMedia({
          video: {
            facingMode: 'user',
            width: { ideal: 320 }, // Reduced from 640 for iOS performance
            height: { ideal: 240 }  // Reduced from 480 for iOS performance
          }
        });
